...
class MangaDexAPI:
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(headers={'User-Agent': 'Mozilla/5.0'}, timeout=timeout)
        return self
    ...
    async def get_manga_info(self, manga_id: str) -> Optional[Dict]:
        try:
            ...
            return {
                'id': manga_id, 'title': title, 'cover_url': cover_url,
                'authors': authors, 'artists': artists,
                'status': status_raw.title(), 'year': attrs.get('year'),
                'last_volume': attrs.get('lastVolume'), 'last_chapter': attrs.get('lastChapter'),
                'tags': tags[:10],
            }
        except Exception as e:
    ...
    async def get_latest_chapters(self, offset: int = 0) -> List[Dict]:
        try:
            params = {
                "limit": 20, "offset": offset, "translatedLanguage[]": ["en"],
                "order[publishAt]": "desc", "includes[]": ["manga", "scanlation_group"],
            ...
            for item in data["data"]:
                ...
                if not manga_id:
                    continue
                chapters.append({
                    "id": item["id"], "manga_id": manga_id, "manga_title": manga_title,
                    "number": attrs.get("chapter") or "0", "chapter": attrs.get("chapter") or "0",
                    "title": attrs.get("title") or "", "url": item["id"], "group": group, "source": "mangadex",
                })
            return chapters
        except Exception as e:
    ...
    async def search_manga(self, query: str, limit: int = 25) -> List[Dict]:
        try:
            params = {
                "title": query, "limit": min(int(limit or 25), 100),
            ...
            for item in data["data"]:
                ...
                title = title_obj.get("en") or next(iter(title_obj.values()), "Unknown")
                results.append({"id": item["id"], "title": title, "status": attrs.get("status"), "year": attrs.get("year")})
            return results
        except Exception as e:
    ...
    async def get_manga_chapters(self, manga_id: str, limit: int = 20, offset: int = 0, languages=None):
        ...
        try:
            params = {
                ...
                "translatedLanguage[]": languages, "order[chapter]": "asc",
                "contentRating[]": ["safe", "suggestive", "erotica"],
            }
            data = await self.api_request("/chapter", params)
            if data is None:
    ...
    async def get_chapter_images(self, chapter_id: str) -> List[str]:
        try:
            ...
            hash_ = ch.get("hash")
            files = ch.get("data") or ch.get("dataSaver") or []
            if not base or not hash_ or not files:
                return []
            quality = "data" if ch.get("data") else "data-saver"
            return [f"{base}/{quality}/{hash_}/{f}" for f in files]
        except Exception as e:
            logger.error(f"get_chapter_images: {e}")
            return []
